...
static lcd_send_finish_callback_t lcd_send_finish_callback;
...
static void lcd_nvic_init(void)
{
...
static void lcd_lowlevel_init(void)
{
    ...
    SPI_InitStructure.SPI_CPOL = SPI_CPOL_High;    // low,两组
    SPI_InitStructure.SPI_CPHA = SPI_CPHA_2Edge;   // 1Edge
...
{
    DMA_DeInit(DMA1_Channel3);              //恢复到默认状态
...
{
    lcd_send_finish_callback = callback;
}
...
void DMA1_Channel3_IRQHandler(void)
{
    if (DMA_GetITStatus(DMA1_IT_TC3) == SET)
    {
        ...
        if (lcd_send_finish_callback)
            lcd_send_finish_callback();
